...
const MAX_DIM = 1600; // то же ограничение, что и в web handlePhoto()
...
export default function PhotoScreen() {
  ...
  const pickFrom = async (source: 'camera' | 'library') => {
    ...
    // allowsEditing убран специально — на вебе фото уходит целиком, без
    // принудительной обрезки; форсированный crop здесь мог срезать сам
    // материал из кадра и давал ложное "not a construction material".
    const opts: ImagePicker.ImagePickerOptions = { quality: 1, allowsEditing: false };
    const res =
    ...
    try {
      // Пересобираем фото сами (ресайз до 1600px + JPEG q0.82) — точно
      // так же, как web-версия делает через canvas.toDataURL(). Это даёт
      // гарантированно валидный JPEG независимо от того, что реально
      // вернула камера/галерея (HEIC, PNG, live photo и т.д.)
      ...
      alert("Couldn't process this photo — please try a different one.");
    }
  };
  ...
  const analyze = async () => {
    ...
    setLoading(true);
    try {
      const r = await analyzePhotoBase64(base64, zip.trim() || '77001', qty.trim(), 'image/jpeg');
      setResult(r);
    } catch (e) {
